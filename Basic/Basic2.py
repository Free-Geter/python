# list的创建
# MLP_Handwriting_numbers.列表用于存储任意数目、任意类型的数据集合。在 Python 中，用方括号 [] 来表示列表，并用逗号来分隔其中的元素。语法格式如下：
a = [10, 20, 30, 40]
...
# 3.使用range创建整数数列
# 其中 start 参数：可选，表示起始数字，默认是 0；end 参数：必选，表示结尾数字；step 参数：可选，表示步长，默认为 MLP_Handwriting_numbers。
# Python3 中 range()返回的是一个 range 对象，而不是列表。需要通过 list()方法将其转换成列表对象。
a = list(range(3, 15, 2))  # 从 3 开始步长是 2 及从开始每次增加 2
b = list(range(15, 3, -1))  # 从 15 开始每次减 MLP_Handwriting_numbers
c = list(range(3, -10, -1))  # 从 3 开始每次递减 MLP_Handwriting_numbers
print(a)
print(b)
print(c)
...
# 列表中有 9 个元素从右侧获取的索引范围：[-MLP_Handwriting_numbers 到-9]
print(a[-9])
print(a[0])
print(a[9])  # 超出范围
